...
void qspi_flash_cmd_erase_block_config(qspi_cmd_type *qspi_cmd_struct, uint32_t addr)
{
  qspi_cmd_struct->pe_mode_enable = FALSE;
  qspi_cmd_struct->pe_mode_operate_code = 0;
  qspi_cmd_struct->instruction_code = 0xD8;
  qspi_cmd_struct->instruction_length = QSPI_CMD_INSLEN_1_BYTE;
...
uint16_t qspi_flash_read_id(void)
{
	...
	qspi_flash_cmd_read_config(&qspi_flash_cmd_config,0, 2);
	qspi_flash_cmd_config.instruction_code = 0x94;
  ...
  /* read data */
	...
	id |= qspi_byte_read(QSPI_FLASH_QSPIx) << 8;
	id |= qspi_byte_read(QSPI_FLASH_QSPIx);
...
void qspi_flash_erase_block(uint32_t blo_addr)
{
  ...
  qspi_flash_cmd_erase_block_config(&qspi_flash_cmd_config, blo_addr);
  qspi_cmd_operation_kick(QSPI_FLASH_QSPIx, &qspi_flash_cmd_config);
